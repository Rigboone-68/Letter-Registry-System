...
The rule itself (brief §2, §4, §8):

  * SYSTEM_ADMIN: always allowed, regardless of `department_id` — including
    when it's `None`. SYSTEM_ADMIN's own `department_id` is always `None`
    (enforced by `ck_users_role_department_pairing`, Phase 2), so this rule
    is checked *before* anything ever compares `user.department_id` to the
    target — no code path here assumes `user.department_id` is set.
  * ADMIN / USER: allowed only if `department_id` equals their own
    ...
    `DepartmentAccessDeniedError`'s docstring).
"""
